...
class Operaton_DMN_Admin {
    ...
    public function ajax_test_endpoint() {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Operaton DMN Admin: Testing endpoint connectivity');
        }
        
        // Verify nonce and permissions
        if (!wp_verify_nonce($_POST['nonce'], 'operaton_test_endpoint') || !current_user_can($this->capability)) {
        ...
        // Test the endpoint with OPTIONS request first
    ...
    public function ajax_test_full_config() {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Operaton DMN Admin: Testing full configuration');
        }
        ...
        // Use the API class to test the full configuration
        $test_result = $this->core->test_full_endpoint_configuration($base_endpoint, $decision_key);
    ...
    public function ajax_clear_update_cache() {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Operaton DMN Admin: Clearing update cache');
        }
        
        if (!current_user_can($this->capability) || !wp_verify_nonce($_POST['_ajax_nonce'], 'operaton_admin_nonce')) {
            wp_send_json_error(array('message' => __('Insufficient permissions', 'operaton-dmn')));
        }
